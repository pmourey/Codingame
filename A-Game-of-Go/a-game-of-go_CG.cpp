...
public:
  ...
  PieceGroup(int x, int y, PieceColor col) {
    color = col;
    locations.push_back(y * 19 + x); // easier than storing two different
                                     // numbers
  }
  ...
  void addPiece(int x, int y) { locations.push_back(y * 19 + x); }
...
public:
  Board(int size) {
    vector<vector<PieceColor>> board(size,
                                     vector<PieceColor>(size)); // the board
